...
export const useSupplierLock = () => {
  const { showToast } = useToast();
  const queryClient = useQueryClient();

  const useLockList = (filters: SupplierLockFilterParams) =>
    useInfiniteQuery({
      queryKey: ["supplier-lock-list", filters],
      queryFn: ({ pageParam = 1 }) =>
  ...
  const useLockDetail = (id?: string) =>
    useQuery({
      queryKey: ["supplier-lock-detail", id],
  ...
  const useApprove = () =>
    useMutation({
      ...
      onSuccess: (res: any) => {
        ...
        queryClient.invalidateQueries({ queryKey: ["supplier-lock-list"] });
        queryClient.invalidateQueries({ queryKey: ["supplier-lock-detail"] });
      },
      onError: (err: any) => {
        showToast({
          type: "danger",

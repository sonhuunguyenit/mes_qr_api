...
interface SupplierLawFilterSheetProps {
  initialFilters: SupplierLawFilterParams;
  onApply: (filters: SupplierLawFilterParams, isReset?: boolean) => void;
  onClose: () => void;
}

const SupplierLawFilterSheet = ({
  initialFilters,
  onApply,
  onClose,
}: SupplierLawFilterSheetProps) => {
  const { spacing, colors } = useTheme();
  const [filters, setFilters] =
    useState<SupplierLawFilterParams>(initialFilters);
  ...
  const handleReset = () => {
    const resetFilters: SupplierLawFilterParams = {
      pageIndex: 1,
      pageSize: 10,
      supplierName: "",
      taxCode: "",
  ...
  const updateFilter = <K extends keyof SupplierLawFilterParams>(
    key: K,
    value: SupplierLawFilterParams[K],
  ) => {
    setFilters((prev: SupplierLawFilterParams) => ({ ...prev, [key]: value }));
  };
...
export default React.memo(SupplierLawFilterSheet);

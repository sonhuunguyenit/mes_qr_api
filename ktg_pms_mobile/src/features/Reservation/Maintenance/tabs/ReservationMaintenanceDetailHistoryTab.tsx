...
import { ReservationMaintenanceHistoryDetailSheet } from "../sheets/ReservationMaintenanceHistoryDetailSheet";

interface Props {
  data: ReservationDetailData;
}

export const ReservationMaintenanceDetailHistoryTab = ({ data }: Props) => {
  ...
  const handleRowDoublePress = useCallback(
    (index: number) => {
      ...
      if (item) {
        openSheet(<ReservationMaintenanceHistoryDetailSheet item={item} index={index} />);
      }
    },
